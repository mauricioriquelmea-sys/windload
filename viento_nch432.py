# -*- coding: utf-8 -*-
import streamlit as st
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
import base64
import os
import math

# 1. CONFIGURACIÓN Y LOGO CORPORATIVO
st.set_page_config(page_title="NCh 432-2025 | Proyectos Estructurales", layout="wide")

def render_logo(image_file):
    if os.path.exists(image_file):
        with open(image_file, "rb") as f:
            data = f.read()
            url = base64.b64encode(data).decode()
        st.markdown(f'<div style="text-align: center;"><img src="data:image/png;base64,{url}" width="500"></div>', unsafe_allow_html=True)
    else:
        st.title("🏗️ Proyectos Estructurales EIRL")

render_logo("Logo.png")
st.subheader("Análisis Integral de Presiones: Cubiertas y Fachadas (NCh 432-2025)")

# 2. ENTRADA DE DATOS (SIDEBAR)
st.sidebar.header("⚙️ Parámetros de Diseño")

# --- MÓDULO AVANZADO: GUÍA DE VELOCIDAD SEGÚN NCh 432 ---
with st.sidebar.expander("🚩 Determinar Velocidad Básica (V)"):
    st.write("**Zonificación y Valores para Chile:**")
    
    # Tabla 1 Integrada
    tabla_v = {
        "Zona": ["I-A", "I-B", "II-A", "II-B", "III-A", "III-B", "IV-A", "IV-B", "V", "VI"],
        "Altitud (msnm)": ["< 2000", "≥ 2000", "< 1500", "≥ 1500", "< 1000", "≥ 1000", "< 800", "≥ 800", "-", "-"],
        "V (m/s)": [27, 30, 27, 35, 34, 35, 37, 40, 40, 44]
    }
    st.table(pd.DataFrame(tabla_v))
    
    # Botón para mostrar mapas de zonificación
    if st.button("Ver Mapas de Zonificación"):
        st.info("Visualizando Figuras 2 - Zonificación de Viento")
        # Mostramos los mapas cargados en el repo
        for img in ["F2.png", "F3.png", "F4.png", "F5.jpg"]:
            if os.path.exists(img):
                st.image(img, caption=f"Referencia Norma NCh 432: {img}")

V = st.sidebar.number_input("Velocidad básica V (m/s)", value=35.0)
H_edif = st.sidebar.number_input("Altura promedio edificio H (m)", value=12.0)
theta = st.sidebar.slider("Inclinación de Techo θ (°)", 0, 45, 10)

# Geometría y Regla de 1/3
st.sidebar.subheader("📐 Geometría del Elemento")
l_elem = st.sidebar.number_input("Largo del elemento (m)", value=3.0)
w_input = st.sidebar.number_input("Ancho tributario real (m)", value=1.0)
w_trib = max(w_input, l_elem / 3)
area_efectiva = l_elem * w_trib

# Factor Topográfico Riguroso
with st.sidebar.expander("🏔️ Factor Topográfico (Kzt)"):
    metodo = st.radio("Método", ["Manual", "Calculado"])
    if metodo == "Manual":
        Kzt_val = st.number_input("Valor Kzt directo", value=1.0)
    else:
        tipo_relieve = st.selectbox("Forma de relieve", ["Escarpe 2D", "Colina 2D", "Colina 3D"])
        H_colina = st.number_input("Altura H (m)", value=27.0)
        Lh = st.number_input("Distancia Lh (m)", value=1743.7)
        x_dist = st.number_input("Distancia x (m)", value=0.0)
        z_alt = st.number_input("Altura z (m)", value=10.0)
        
        k1_b, g, m = (0.75, 2.5, 1.5) if tipo_relieve == "Escarpe 2D" else (1.05, 1.5, 1.5) if tipo_relieve == "Colina 2D" else (0.95, 1.5, 4.0)
        k1 = k1_b * (H_colina / Lh); k2 = (1 - abs(x_dist) / (m * Lh)); k3 = math.exp(-g * z_alt / Lh)
        Kzt_val = (1 + k1 * k2 * k3)**2
        st.info(f"Kzt: {Kzt_val:.3f}")

exp_cat = st.sidebar.selectbox("Exposición", ['B', 'C', 'D', 'A'], index=1)
imp_cat = st.sidebar.selectbox("Importancia", ['I', 'II', 'III', 'IV'], index=2)

# 3. MOTOR MATEMÁTICO (5 ZONAS)
def get_gcp(area, g1, g10):
    if area <= 1.0: return g1
    if area >= 10.0: return g10
    return g1 + (g10 - g1) * (np.log10(area) - np.log10(1.0))

# Interpolación y qh
imp_map = {'I': 0.87, 'II': 1.0, 'III': 1.15, 'IV': 1.15}
exp_params = {'A': [5.0, 457.0], 'B': [7.0, 366.0], 'C': [9.5, 274.0], 'D': [11.5, 213.0]}
alpha, zg = exp_params[exp_cat]
kz = 2.01 * ((max(H_edif, 4.6) / zg)**(2/alpha))
qh = (0.613 * kz * Kzt_val * 0.85 * (V**2) * imp_map[imp_cat]) * 0.10197
gc_pi = 0.18

# Coeficientes de las 5 zonas
if theta <= 7:
    z1, z2, z3 = get_gcp(area_efectiva, -1.0, -0.9), get_gcp(area_efectiva, -1.8, -1.1), get_gcp(area_efectiva, -2.8, -1.1)
else:
    z1, z2, z3 = get_gcp(area_efectiva, -0.9, -0.8), get_gcp(area_efectiva, -1.3, -1.2), get_gcp(area_efectiva, -2.0, -1.2)
z4, z5 = get_gcp(area_efectiva, -1.1, -0.8), get_gcp(area_efectiva, -1.4, -1.1)

# 4. RESULTADOS Y GRÁFICO
col1, col2 = st.columns([1, 1.2])
with col1:
    st.metric("Presión qh", f"{qh:.2f} kgf/m²")
    df = pd.DataFrame({
        "Zona": ["Zona 1 (Techo)", "Zona 2 (Techo)", "Zona 3 (Techo)", "Zona 4 (Muro)", "Zona 5 (Muro)"],
        "Ubicación": ["Centro", "Borde", "Esquina", "Estándar", "Esquina"],
        "GCp": [round(z, 3) for z in [z1, z2, z3, z4, z5]],
        "Presión Diseño (kgf/m²)": [round(qh*(z-gc_pi), 2) for z in [z1, z2, z3, z4, z5]]
    })
    st.table(df)

with col2:
    areas = np.logspace(0, 1, 50)
    fig, ax = plt.subplots(figsize=(7, 4.5))
    ax.plot(areas, [get_gcp(a, -1.4, -1.1) for a in areas], label='Zona 5 (Esquina Muro)', color='red', lw=2)
    ax.plot(areas, [get_gcp(a, -1.1, -0.8) for a in areas], label='Zona 4 (Estandar Muro)', color='green', lw=2)
    ax.scatter([area_efectiva, area_efectiva], [z4, z5], color='black', zorder=5)
    ax.set_title("Sensibilidad GCp en Fachadas (Interpolación Log)")
    ax.set_xlabel("Área Tributaria (m²)"); ax.set_ylabel("GCp"); ax.grid(True, alpha=0.3); ax.legend()
    st.pyplot(fig)

# 5. CONTACTO
st.markdown("---")
st.caption("Cálculo bajo normativa NCh 432-2025. Proyectos Estructurales EIRL.")
st.markdown(f'<div style="text-align: right;"><a href="mailto:mriquelme@proyectosestructurales.com">mriquelme@proyectosestructurales.com</a></div>', unsafe_allow_html=True)